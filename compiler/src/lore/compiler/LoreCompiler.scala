...
object LoreCompiler {
  ...
  /**
    ...
    * list of feedback.
    */
  def compile(fragments: Vector[Fragment], options: CompilerOptions)(implicit reporter: Reporter): (Registry, Option[String]) = {
    ...
    // erroneous program.
    val registry = analyze(fragments)
    val code = if (!reporter.hasErrors) {
      Some(generate(registry, options))
    } else None
  ...
  /**
    ...
    *
    * Compilation will never terminate in phase 1 or 2, because we can always produce a Registry from a subset of the
    * program.
    */
  def analyze(fragments: Vector[Fragment])(implicit reporter: Reporter): Registry = {
