...
object GlobalVariableFeedback {
  case class NameTaken(variable: GlobalVariableDefinition) extends Feedback.Error(variable) {
    override def message: String = s"The name of the global variable `${variable.name}` is already taken by a module" +
      s" or multi-function. Modules, global variables, and multi-functions may not share names."
  }
