...
import lore.compiler.core.{Error, Position}
import lore.compiler.semantics.functions.{FunctionDefinition, MultiFunctionDefinition}
import lore.compiler.types.Type

object DispatchFeedback {

  case class EmptyFit(mf: MultiFunctionDefinition, inputType: Type, override val position: Position) extends Error(position) {
  ...
  case class AmbiguousCall(mf: MultiFunctionDefinition, inputType: Type, min: Vector[FunctionDefinition], override val position: Position) extends Error(position) {
