...
object ModuleFeedback {
  case class NameTaken(module: GlobalModule) extends Feedback.Error(module.positions.head) {
  ...
  object Import {
    ...
    object Wildcard {
      case class NotFound(node: ImportNode, absolutePath: NamePath) extends Feedback.Error(node) {
        override def message: String = s"This wildcard import cannot be resolved: `$absolutePath` does not exist or" +
          s" is not a module."
      }
    }
  }
