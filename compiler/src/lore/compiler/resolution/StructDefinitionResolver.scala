...
object StructDefinitionResolver {
  ...
  def resolve(node: DeclNode.StructNode)(implicit types: Registry.Types, terms: Registry.Terms, reporter: Reporter): StructDefinition = {
    Resolver.withRegistryScopes(node.localModule) {
      registryTypeScope => implicit termScope =>
        ...
        val companionModule = terms.modules.get(node.fullName)
  ...
  private def resolveProperty(node: DeclNode.PropertyNode)(implicit typeScope: TypeScope, termScope: TermScope, reporter: Reporter): StructPropertyDefinition = {
