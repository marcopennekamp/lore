...
/**
  ...
  *
  * TODO (inference): Can't we use subtyping unification to achieve the same goal? We'd still need a run-time type
...
class TypeVariableAllocation(variables: Set[TypeVariable]) {
  ...
  /**
    * Returns all assignments contained in the current allocation without checking for consistency.
    */
  def allAssignments: Map[TypeVariable, Vector[Type]] = allocation.toMap
...
object TypeVariableAllocation {
  ...
  private def assign(t1: Type, t2: Type)(implicit allocation: TypeVariableAllocation): Unit = {
    ...
    (t1, t2) match {
      ...
      // inheritance is handled. An example:
      //    trait A[+X]
