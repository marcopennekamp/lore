...
import lore.compiler.poem.{PoemFunction, PoemNamedType, PoemSchema}
...
object DeclaredSchemaAssembler {
  ...
  /**
    ...
    * constructor.
    */
  def generate(schema: DeclaredSchema)(implicit registry: Registry): (PoemSchema, Vector[PoemFunction]) = {
    ...
    schema match {
      case schema: TraitSchema => (TraitSchemaAssembler.generate(schema, poemTypeParameters, poemSupertraits), Vector.empty)
