...
/**
  * A binding scope backed by the registry and a local module for name resolution. It contains modules, global
  * variables, multi-functions, struct bindings, and objects.
  *
...
case class LocalModuleBindingScope(
  ...
  override protected def local(name: String): Option[Binding] = {
    localModule.getAbsolutePath(name, NameKind.Binding).flatMap(global)
  }
