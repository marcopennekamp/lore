...
trait ExpressionIdentityVisitor[R] extends ExpressionVisitor[Expression, R] {
  ...
  override def visit(expression: TupleValue)(values: Vector[Expression]): R = wrap(expression.copy(values))
  ...
  override def visit(expression: ListValue)(values: Vector[Expression]): R = wrap(expression.copy(values))
