...
sealed trait MemberDefinition extends Positioned {
  ...
  def asVirtualMember: VirtualMember = semantics.VirtualMember(name, tpe, isComponent = isComponent, isMutable = isMutable, underlying = Some(this))
}
...
class ComponentDefinition(
  override val name: String, override val tpe: ClassType, val overrides: Option[String], override val position: Position,
) extends MemberDefinition {
