...
object LexicalParser {
  ...
  /**
    ...
    * such as `return` being possible in a declaration but not as a simple expression, as the parser will prefer to
    * read `return` as a return top-level expression.
    *
  ...
  def typeIdentifier[_: P]: P[String] = P(!criticalKeyword ~ (typeSpecialCharacter | letter | "_") ~ (typeSpecialCharacter | letter | digit | "_").rep).!
