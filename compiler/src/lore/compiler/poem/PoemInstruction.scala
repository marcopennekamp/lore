...
object PoemInstruction {
  ...
  def stringify(instruction: PoemInstruction): String = {
    s"${instruction.operation} " + (instruction match {
      ...
      case PropertyGet(target, instanceKind, instance, propertyName) => s"$target <- $instance[$propertyName] (Instance kind: $instanceKind)"
      case PropertySet(instanceKind, instance, propertyName, value) => s"$instance.$propertyName <- $value (Instance kind: $instanceKind)"
      ...
      case CallPoly(target, mf, typeArguments, valueArguments) =>
        val typeArgumentString = if (typeArguments.nonEmpty) s"[${typeArguments.mkString(", ")}]" else ""
        s"$target <- $mf$typeArgumentString(${valueArguments.mkString(", ")})"
