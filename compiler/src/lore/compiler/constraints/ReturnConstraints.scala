...
private class ReturnDeadCodeVisitor(implicit reporter: Reporter) extends CombiningTopLevelExprVisitor[DefinitelyReturns] {
  ...
  override def visit(node: TopLevelExprNode, returns: Vector[DefinitelyReturns]): DefinitelyReturns = node match {
    ...
    case ExprNode.LambdaValueNode(_, _, _) =>
      // The body of a lambda function doesn't affect the outer control flow, and so a return occurring in a lambda
      // function body does not lead to dead code.
      false
