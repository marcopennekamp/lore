...
- Equality and ordering:
  - Well-defined equality and order semantics for all kinds of types, including shapes, tuples, lists, maps, and so on, using `are_equal?` and `less_than?`. These functions should be specializable for specific types, such as specific lists and maps.
    - Tuple and shape equality either has to be defined within the VM or we need a mechanism for iterating over tuple elements and shape properties (basically reflection).
    - Shapes should have no default ordering.
  - Default equality for structs.
    - This requires the compiler to generate a default `are_equal?` for a struct, UNLESS such a function is already defined. This is a slippery slope, as multi-functions are "freeform". We could choose a "derive" tactic similar to Rust, which would play well with `are_equal?` requiring an explicit equality trait/protocol, as the "derive" could simultaneously instruct the compiler to generate a default `are_equal?` as well as extend the equality trait/protocol.
    - Structs should have no default ordering.
