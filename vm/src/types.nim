...
proc sum*(parts: open_array[Type]): SumType = SumType(kind: Kind.Sum, parts: new_immutable_seq(parts))
proc intersection*(parts: open_array[Type]): IntersectionType = IntersectionType(kind: Kind.Intersection, parts: new_immutable_seq(parts))
proc tpl*(elements: open_array[Type]): TupleType = TupleType(kind: Kind.Tuple, elements: new_immutable_seq(elements))
