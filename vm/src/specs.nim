...
type
  SpecPurpose {.pure.} = enum Test, Benchmark
...
type
  SpecGroup = ref object
    ## A spec group combines all specs of the same module.
    module_name: string
    specs: seq[Spec]

iterator spec_groups(universe: Universe, purpose: SpecPurpose): SpecGroup =
  ## A spec group view on the specs of `universe`. The specs already have to be sorted by module name, so the resulting
  ## spec groups will also be sorted by default. Specs are filtered by `include_tests` and `include_benchmarks`. Spec
  ## groups without a single included spec aren't yielded.
...
proc run_tests*(universe: Universe) =
  ...
  with_frame_mem(proc (frame_mem: pointer) =
    for spec_group in universe.spec_groups(SpecPurpose.Test):
