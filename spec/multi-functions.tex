...
To define multiple dispatch formally, we first need an operation that allows us to reduce the set of multi-function instances to only those functions that could be called with a given tuple of arguments, represented by a type bound.

\todo{Shouldn't we dispatch on the whole function type (including the output) to check if the covariant return type also satisfies the type bound? Or do we enforce that output types are covariant in respect to all possible function definitions?}

We define a function $\mffit : \typeset \rightarrow \mfset \rightarrow \powerset{\funcset}$ as follows:
\begin{equation*}
	\mffit(b)((\wildcard, F)) = \{ f \in F \mid \funcin(f) \geq b \}
\end{equation*}

\noindent That is, we look at all functions $f \in F$ and choose only those whose input type is a supertype of the given type bound $b$. We choose the supertyping relation and not the subtyping relation, because function inputs are contravariant and thus the subtyping relation regarding function types is reversed in the input.
