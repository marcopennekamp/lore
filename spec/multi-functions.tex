...
\begin{lstlisting}
    function search(list: List[a]): a = // f1
      // search with linear search
    function search(list: List[a] & Sorted): a = // f2
      // search with binary search
\end{lstlisting}

\noindent That is, we can encode that some lists are sorted in the type system, and for those lists we can specialise the \texttt{search} function so it uses a more efficient algorithm, binary search instead of linear search.
