...
\textbf{Problem with the definition:} The above definition does not consider intersection types. There is a limited, but \textbf{very large} set of \textit{direct subtypes} if you consider intersection types to be direct subtypes. That is, consider a type \type{A} and a label type \type{L}. Trivially, $\type{A \& L} < \type{A}$, and specifically, there is no other type that could be sandwiched between the two. But of course, it makes no sense to require an implementation. A value of type \type{A \& L} will be handled just like a value of the base type. 

We have one interesting property: Since \type{A} must be abstract, \type{A \& L} is also abstract. But we can't blanket-remove abstract subtypes from the definition, because a true direct subtype \type{B} of \type{A} may also be abstract.

So perhaps we should change the constraint from an algorithm-like definition rather to a description of what we want to achieve. This can be put much more succinctly: All possible subtypes of an abstract function's input type should be covered by one concrete function implementation. This ensures that all possible values are dispatched to a concrete function implementation.

\begin{definition} \label{def:totality-constraint2}
	...
	\begin{align*}
		\forall s < \funcin(f).\; \neg \typeabstract(s) \implies \exists f' \in \mathcal{F}.\; \neg \funcabstract(f') \land \mffit(s)(\mathcal{F}) \neq \emptyset
	\end{align*}
\end{definition}

\noindent That is, all instantiable subtypes $s$ of $f$'s input type must be covered by at least one concrete function. This ensures that all input values with which the function can ever be called are dispatched to a concrete function.
