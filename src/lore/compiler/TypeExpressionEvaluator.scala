...
object TypeExpressionEvaluator {
  def evaluate(expression: TypeExprNode)(implicit registry: Registry, fragment: Fragment): C[Type] = {
    val eval = evaluate _
    expression match {
      case TypeExprNode.NominalNode(name) => registry.getType(name, expression)
      ...
      case componentNode@TypeExprNode.ComponentNode(underlying) => registry.getType(underlying, componentNode).flatMap {
